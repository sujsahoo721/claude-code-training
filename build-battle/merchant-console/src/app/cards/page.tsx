...
export default async function CardsPage({
  ...
  const filters: CardFilters = {
    status: (CARD_STATUSES.includes(params.status as CardStatus) ? params.status : "all") as CardFilters["status"],
  ...
  const filtered = Boolean(filters.search || filters.merchantId || (filters.status && filters.status !== "all"))
  ...
  return (
    <section aria-label="Virtual cards">
      <div className="flex flex-col justify-between gap-2 px-4 py-6 sm:flex-row sm:items-center sm:p-6">
        ...
        <IssueCard merchants={merchants.map((m) => ({ id: m.id, name: m.name, currency: m.currency }))} />
      </div>
              ...
              <TableRow>
                <TableCell colSpan={7} className="py-16 text-center">
                  <p className="font-medium text-gray-900 dark:text-gray-50">
                    {filtered ? "No cards match these filters" : "No virtual cards yet"}
                  </p>
            ...
            {rows.map((card) => {
              ...
              return (
                <TableRow key={card.id}>
                  <TableCell>
                    <Link href={`/cards/${card.id}`} className="font-medium text-blue-600 hover:underline dark:text-blue-500">
                      {card.nickname}
                    </Link>
                  </TableCell>
                  ...
                  <TableCell><CardStatusActions cardId={card.id} status={card.status} /></TableCell>
                </TableRow>
              )
            })}
          </TableBody>
      ...
      <div className="flex items-center justify-between gap-4 px-4 py-4 sm:px-6">
        <p className="text-sm text-gray-500">{total.toLocaleString()} cards · page {page} of {pageCount}</p>
        <div className="flex gap-2">
          ...
          <Button variant="secondary" className="py-1.5" disabled={page >= pageCount} asChild={page < pageCount}>
